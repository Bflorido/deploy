...
| 15 | Chrome iOS seguía mostrando el lockdown | ✅ Cerrado | Chrome iOS conservaba en caché el `console.html` antiguo, así que no corría ni la guarda del `<head>`. Se añadió **sello de versión** a los recursos (`?v=`) para que el cambio de archivo implique URL nueva, y el README documenta que hay que subir el número en cada despliegue de JS/CSS. |
| 16 | **Los iconos dejaron de abrir en el móvil** (regresión propia) | ✅ Cerrado | Al sustituir el manejador táctil de los iconos por el sintetizador, los `.dicon` quedaron sin efecto: su acción vive en `ondblclick`, y **un click sintético nunca produce un `dblclick`**, así que el handler no se ejecutaba. Además el anti-rebote global de 350 ms descartaba el segundo toque cuando se abrían dos apps seguidas, y un click nativo (Android) dejaba "sordo" el tap siguiente. Ahora la lógica **distingue el tipo de manejador**: `ondblclick` → se ejecuta el handler directamente; `onclick` → se sintetiza el click (y se cede al nativo si acaba de llegar); el anti-rebote es **por elemento**, no global. Verificado con 12 casos (`tools/dev/tap-icon-flow-test.js`). |
| 17 | Compromiso asumido: en Android, un `onclick` inline puede ejecutarse dos veces | ⚠️ Documentado | Chrome Android entrega su click nativo ~300 ms después del toque, cuando ya sinteticé el mío. Los handlers del proyecto son idempotentes (`openWindow`/`closeWindow`/`minimize`/`taskClick`), así que no hay efecto visible; si algún día se añade un `onclick` con efectos acumulativos, habrá que revisarlo. |
